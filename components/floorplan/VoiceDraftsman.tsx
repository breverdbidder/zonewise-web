...
// ─────────────────────────────────────────────────────────────────────────
// ZoneWise Voice Draftsman — custom voice client
// Mirrors the pattern built for BidDeed.AI's "Deed" agent: a raw WebSocket
// connection to ElevenLabs (not the packaged widget), a signed URL fetched
// from a shared Supabase Edge Function so the API key never reaches the
// browser, an email gate before the mic opens, a live bidirectional
// transcript, file attachment support, and a capped session length with an
// in-call upsell nudge partway through.
...
export default function VoiceDraftsman() {
  ...
  const startSession = useCallback(async () => {
    ...
    try {
      ...
      stopSession()
      setStatusMsg('Mic permission denied — try again or contact support.')
      return
    }
    setStatus('connecting')
    setStatusMsg('Connecting to the Voice Draftsman…')
  ...
  const submitGateEmail = useCallback(async () => {
    ...
    fetch(LEAD_ENDPOINT, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ email, source: 'floorplan_voice_gate' }),
    }).catch(() => {})
    startSession()
  ...
  const handleFileSelected = useCallback(async (file: File) => {
    ...
    setAttachProgress({ state: 'ok', msg: `✓ ${file.name} sent${attachCaption.trim() ? ' with caption' : ''}. The draftsman is reviewing it.` })
    setAttachCaption('')
  }, [attachCaption])
      ...
      {status !== 'cap-reached' && (
        <button
          ...
          className={`px-4 py-2 rounded text-sm font-medium transition-colors ${
            status === 'listening' || status === 'connecting' || status === 'requesting-mic'
              ? 'bg-red-600 hover:bg-red-500 text-white'
              : 'bg-blue-600 hover:bg-blue-500 text-white'
          }`}
        >
          {status === 'idle' ? '🎙️ Talk to the Voice Draftsman' : '⏹ Stop'}
        </button>
      )}
      ...
      {showGate && (
        <div className="mt-3 flex gap-2 items-start">
          ...
          <button
            ...
            className="px-3 py-2 rounded bg-blue-600 hover:bg-blue-500 text-white text-sm font-medium"
          >
            Start
          </button>
        </div>
