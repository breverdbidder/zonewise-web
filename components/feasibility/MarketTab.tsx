  ...
  return (
    <div className="flex flex-col lg:flex-row gap-5">
      <div className="flex-1">
        <Card className="p-5">
          <div className="flex items-center mb-4">
            <span className="text-base font-bold text-[rgb(var(--zw-ink))]">{site.county} County Market Intelligence</span>
          ...
          <div className="grid grid-cols-3 gap-3 mb-5">
            {[
              ...
              <div key={label} className="bg-slate-50 rounded-lg p-3.5" style={{ borderLeft: `3px solid ${color}` }}>
                <div className="text-[10px] text-[rgb(var(--zw-ink2))] uppercase tracking-wider">{label}</div>
                <div className="text-xl font-bold text-[rgb(var(--zw-ink))] mt-1" style={{ fontFamily: "'JetBrains Mono', monospace" }}>{val}</div>
                <div className="text-[11px] text-[rgb(var(--zw-ink2))] mt-0.5">{sub}</div>
              </div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-2 gap-3 mb-4">
            ...
            <div className="rounded-lg p-3.5 bg-amber-50">
              ...
              {['Kennedy Space Center', 'Patrick SFB', 'L3Harris Technologies', 'Health First', 'SpaceX / Blue Origin'].map((e) => (
                <div key={e} className="text-xs text-[rgb(var(--zw-ink2))] py-0.5">• {e}</div>
              ))}
            </div>
          </div>
      ...
      <div className="w-full lg:w-[300px] flex-shrink-0">
        ...
        <div className="text-sm font-bold text-[rgb(var(--zw-ink))] mb-3">{site.address}</div>
