...
function ValueRow({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) {
  return (
    <div className="flex justify-between gap-3 text-sm">
      <span className="text-gray-500 dark:text-slate-400 shrink-0">{label}</span>
      <span className={highlight ? 'text-[rgb(var(--zw-brand))] font-semibold' : 'text-gray-800 dark:text-slate-100'}>{value}</span>
    </div>
  )
}
  ...
  return (
    ...
    >
      ...
      >
        ...
        <button
          onClick={onClose}
          className="absolute top-3 right-3 z-10 w-8 h-8 flex items-center justify-center rounded-full bg-gray-100 dark:bg-slate-800 hover:bg-gray-200 dark:hover:bg-[rgb(var(--zw-elev))] text-gray-500 dark:text-slate-400 hover:text-gray-900 dark:hover:text-[rgb(var(--zw-ink))] transition-colors"
        ...
        <div className="shrink-0">
          ...
          {(data.photoUrl || data.proxyPhotoUrl) ? (
            <div className="relative w-full h-44 sm:h-52 overflow-hidden rounded-t-2xl sm:rounded-t-2xl">
              ...
              />
              <div className="absolute inset-0 bg-gradient-to-t from-[rgb(var(--zw-page))] via-transparent to-transparent" />
            </div>
          ) : (
            <div className="w-full h-28 rounded-t-2xl bg-gradient-to-br from-[rgb(var(--zw-elev))] to-gray-300 dark:to-slate-800 flex items-center justify-center">
              <Home className="w-12 h-12 text-[rgb(var(--zw-brand)/0.4)]" />
            </div>
          )}
          ...
          <div className="px-4 pt-3 pb-3 border-b border-gray-200 dark:border-slate-700">
            <div className="flex items-start justify-between gap-3">
              ...
              <span className="shrink-0 mt-0.5 px-2 py-0.5 rounded-md bg-[rgb(var(--zw-elev))] text-[rgb(var(--zw-brand))] text-xs font-mono font-semibold whitespace-nowrap">
                {displayParcelId}
              </span>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3">
          ...
          <div className="flex gap-2 pt-1 pb-2">
            {taxAcct && (
              <a
                ...
                className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg bg-[rgb(var(--zw-brand))] hover:bg-[rgb(var(--zw-brand))]/80 text-white text-xs font-medium transition-colors"
              >
          ...
          <div className="pb-2">
            <Link
              ...
              className="w-full flex items-center justify-center gap-1.5 px-3 py-2.5 rounded-lg bg-[rgb(var(--zw-brand))] hover:bg-amber-500 text-white text-xs font-semibold transition-colors"
            >
