  ...
  if (collapsed) {
    return (
      <div className="w-16 bg-white dark:bg-slate-900 border-r border-gray-200 dark:border-slate-800 flex flex-col items-center py-4 gap-4 transition-colors">
        <button onClick={onToggleCollapse} className="p-2 hover:bg-gray-100 dark:hover:bg-[rgb(var(--zw-card))] rounded-lg transition-colors" title="Expand sidebar">
          <svg className="w-5 h-5 text-gray-500 dark:text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 5l7 7-7 7M5 5l7 7-7 7" />
          </svg>
        </button>
        <button onClick={onNewSession} className="p-2 bg-[rgb(var(--zw-elev))] hover:bg-[rgb(var(--zw-page))] rounded-lg transition-colors" title="New chat">
          <svg className="w-5 h-5 text-[rgb(var(--zw-ink))]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
          </svg>
        </button>
        <button onClick={toggleTheme} className="p-2 hover:bg-gray-100 dark:hover:bg-[rgb(var(--zw-card))] rounded-lg transition-colors" title={`Switch to ${theme === 'dark' ? 'light' : 'dark'} mode`}>
          {theme === 'dark' ? (
        ...
        <Link href="/" className="p-2 hover:bg-gray-100 dark:hover:bg-[rgb(var(--zw-card))] rounded-lg transition-colors" title="Home">
          <svg className="w-5 h-5 text-gray-500 dark:text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="p-4 border-b border-gray-200 dark:border-slate-800">
        <div className="flex items-center justify-between mb-4">
          <Link href="/" className="flex items-center gap-2">
            <div className="w-8 h-8 bg-gradient-to-br from-[rgb(var(--zw-card))] to-[rgb(var(--zw-page))] rounded-lg flex items-center justify-center shadow-lg shadow-zw-navy-500/20">
              <span className="text-[rgb(var(--zw-ink))] font-bold text-sm">Z</span>
            </div>
            <span className="font-semibold text-gray-900 dark:text-slate-100">ZoneWise.AI</span>
          </Link>
          <div className="flex items-center gap-1">
            <button onClick={toggleTheme} className="p-1.5 hover:bg-gray-100 dark:hover:bg-[rgb(var(--zw-card))] rounded-lg transition-colors" title={`Switch to ${theme === 'dark' ? 'light' : 'dark'} mode`}>
              {theme === 'dark' ? (
            ...
            <button onClick={onToggleCollapse} className="p-1.5 hover:bg-gray-100 dark:hover:bg-[rgb(var(--zw-card))] rounded-lg transition-colors">
        ...
        <button onClick={onNewSession} className="w-full flex items-center justify-center gap-2 px-4 py-2.5 bg-[rgb(var(--zw-elev))] hover:bg-[rgb(var(--zw-page))] text-[rgb(var(--zw-ink))] rounded-lg font-medium transition-all hover:shadow-lg hover:shadow-zw-navy-500/20">
                ...
                <button key={session.id} onClick={() => onSelectSession(session)}
                  className={`w-full text-left px-3 py-2.5 rounded-lg transition-all group ${
                    activeSession?.id === session.id
                      ? 'bg-zw-navy-50 dark:bg-zw-navy-600/20 text-[rgb(var(--zw-ink))] dark:text-slate-100 border border-[rgb(var(--zw-border2))] dark:border-zw-navy-500/30'
                      : 'hover:bg-gray-100 dark:hover:bg-[rgb(var(--zw-card)/0.7)] text-gray-700 dark:text-slate-300'
                  }`}>
                  <div className="flex items-start gap-2">
                    <svg className={`w-4 h-4 mt-0.5 flex-shrink-0 ${activeSession?.id === session.id ? 'text-[rgb(var(--zw-ink2))] dark:text-zw-navy-400' : 'text-gray-400 dark:text-slate-500'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="p-4 border-t border-gray-200 dark:border-slate-800">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 bg-gradient-to-br from-gray-400 dark:from-slate-600 to-gray-500 dark:to-slate-700 rounded-full flex items-center justify-center">
            <span className="text-sm font-medium text-[rgb(var(--zw-ink))]">{user?.email?.charAt(0).toUpperCase()}</span>
          </div>
          ...
          <button onClick={onSignOut} className="p-2 hover:bg-gray-100 dark:hover:bg-[rgb(var(--zw-card))] rounded-lg transition-colors" title="Sign out">
        ...
        <div className="mt-3">
          ...
          <div className="h-1.5 bg-gray-200 dark:bg-slate-800 rounded-full overflow-hidden">
            <div className="h-full bg-gradient-to-r from-[rgb(var(--zw-card))] to-[rgb(var(--zw-elev))] rounded-full transition-all"
