...
interface Props {
  ...
  onToggleZoningOverlay: (on: boolean) => void
}
      ...
      <div className="space-y-1.5">
        ...
        <label className="flex items-center gap-2 text-[11px] text-slate-400 cursor-pointer hover:text-slate-200 min-h-[44px] md:min-h-0">
          <input
            ...
            onChange={e => onToggleZoningOverlay(e.target.checked)}
            className="accent-amber-500 w-3 h-3"
          />
          <span className="w-2.5 h-2.5 rounded-sm shrink-0 bg-gradient-to-r from-green-500 via-blue-500 to-red-500" />
          Zoning Overlay
        </label>
