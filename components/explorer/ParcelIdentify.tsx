...
export default function ParcelIdentify({ parcel, onClose }: Props) {
  ...
  return (
    <div className="bg-[rgb(var(--zw-page))] border border-amber-500/30 rounded-xl p-3 shadow-xl w-full">
      {/* Header */}
      <div className="flex items-start justify-between gap-2 mb-2">
        <div className="min-w-0">
          <h3 className="text-sm font-bold text-[rgb(var(--zw-ink))] leading-tight truncate">
            {addr || 'Unknown Address'}
          </h3>
          <p className="text-[11px] text-[rgb(var(--zw-ink2))] mt-0.5">
            {(parcel.CITY || '').trim()}, FL {parcel.ZIP_CODE || ''}
          </p>
          <p className="text-[10px] text-[rgb(var(--zw-ink2))] font-mono mt-0.5 truncate">
        ...
        {onClose && (
          <button
            onClick={onClose}
            className="shrink-0 w-6 h-6 flex items-center justify-center text-[rgb(var(--zw-ink2))] hover:text-[rgb(var(--zw-ink2))] text-lg leading-none"
            aria-label="Close"
          >
            ×
          </button>
      ...
      <div className="grid grid-cols-2 gap-1.5 mb-2">
        {[
          ...
          <div key={s.label} className="bg-[rgb(var(--zw-page))] border border-[rgb(var(--zw-border2))] rounded-md p-2">
            <div className="text-[9px] text-[rgb(var(--zw-ink2))] uppercase tracking-wider">{s.label}</div>
            <div className="text-xs font-bold text-[rgb(var(--zw-ink))] font-mono mt-0.5">{s.value}</div>
          </div>
        ))}
      </div>

      {/* Owner */}
      <div className="bg-[rgb(var(--zw-page))] border border-[rgb(var(--zw-border2))] rounded-md px-2.5 py-2 mb-2">
        <div className="text-[9px] text-[rgb(var(--zw-ink2))] uppercase tracking-wider mb-0.5">Owner</div>
        <div className="text-xs font-semibold text-[rgb(var(--zw-ink))] truncate">{parcel.OWNER_NAME1 || '—'}</div>
        {parcel.OWNER_NAME2 && (
          <div className="text-[10px] text-[rgb(var(--zw-ink2))] truncate">{parcel.OWNER_NAME2}</div>
        )}
      </div>
