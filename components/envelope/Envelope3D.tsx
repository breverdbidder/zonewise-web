...
/**
 * Envelope3D — Three.js 3D building envelope renderer
 * Extracted from zonewise-dev-intel-v3.jsx
 *
 * Renders: lot plane, setback lines, buildable envelope box (semi-transparent),
 * floor plates, height limit plane, north arrow compass, grid helper.
 * Supports mouse/touch orbit, scroll zoom, keyboard arrows, auto-rotate when idle.
 */
...
export interface Envelope3DProps {
  ...
  width: number
  height: number
  /** Pass a ref to expose a reset-view callback */
...
export function Envelope3D({
  lotW, lotD, front, side, rear, maxH, maxCov, far, width, height, onResetView
}: Envelope3DProps) {
  ...
  useEffect(() => {
    ...
    scene.fog = new THREE.FogExp2(new THREE.Color(SLATE), 0.003)
    ...
    const lot = new THREE.Mesh(
      lotGeo,
      new THREE.MeshStandardMaterial({ color: 0x1a1a2e, transparent: true, opacity: 0.7 })
    )
    ...
    if (sbW > 0 && sbD > 0) {
      ...
      const corners: [number, number][] = [[-sbW / 2, -sbD / 2], [sbW / 2, -sbD / 2], [-sbW / 2, sbD / 2], [sbW / 2, sbD / 2]]
      corners.forEach(([x, z]) => {
    ...
    if (env.effFP > 0 && env.floors > 0) {
      ...
      wireframe.position.set(0, eH / 2, oZ)
      scene.add(wireframe)
    ...
    function animate() {
      ...
      idleRef.current++
      if (idleRef.current > 180) {
        rotRef.current.theta += 0.002
        updateCamera()
      }
