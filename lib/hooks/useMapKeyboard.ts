...
export function useMapKeyboard({
        ...
        case 'Tab': {
          ...
          if (e.shiftKey) {
            focusedIndex.current =
              focusedIndex.current <= 0 ? parcels.length - 1 : focusedIndex.current - 1;
          } else {
            focusedIndex.current =
              focusedIndex.current >= parcels.length - 1 ? 0 : focusedIndex.current + 1;
          }
  ...
  return {
    mapAriaProps: {
      ...
      tabIndex: 0,
      'aria-label':
        'Interactive zoning map. Use arrow keys to pan, plus and minus to zoom, Tab to cycle parcels, Enter to select, Escape to deselect.',
