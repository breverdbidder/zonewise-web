 ...
 * @param {import('./site-massing-solver').CandidateFootprint} candidate
 * @returns {Buffer}
 */
function exportSiteMassingDXF(parcel, zoning, candidate) {
  ...
  d.drawPolyline(footprintSP, true)
  ...
  // DIMENSIONS — width/depth call-outs. js-dxf has no native DIMENSION
  // entity, so these are drawn as leader lines + text (still a real,
  // parseable DIMENSIONS layer, just not an associative AutoCAD dimension).
  ...
  d.drawText(frontMidX, fp[0][1] - dimOffset - 2, 3, 0, `${widthFt.toFixed(1)} ft`, 'center', 'top')
